...
class ProcessPoolManager:
    ...
    def mark_for_recycle(self) -> int:
        ...
        completing its current execution. Idle processes are recycled immediately.

        Returns:
            Number of processes marked for recycle
        """
        recycled_immediately = 0
        marked_for_later = 0

        for handle in list(self.processes.values()):
            if handle.state == ProcessState.IDLE:
                # Idle process - recycle immediately by spawning async task
                handle.pending_recycle = True
                asyncio.create_task(self._recycle_idle_process(handle))
                recycled_immediately += 1
            else:
        ...
        logger.info(
            ...
            f"({recycled_immediately} immediate, {marked_for_later} after execution)"
        )
