...
class TestSchedules:
    ...
    def test_list_schedules(self, e2e_client, platform_admin):
        """Platform admin can list schedules."""
        response = e2e_client.get(
            "/api/schedules",
            headers=platform_admin.headers,
        )
    ...
    def test_schedules_returns_enriched_metadata(self, e2e_client, platform_admin):
        """Schedules endpoint returns enriched ScheduleMetadata with computed fields."""
        response = e2e_client.get(
            "/api/schedules",
            headers=platform_admin.headers,
        )
        ...
        assert isinstance(schedules, list), "Schedules should return a list"

        # Find the e2e_scheduled_task workflow (has schedule="0 9 * * *")
        scheduled_workflow = next(
            (s for s in schedules if s.get("name") == "e2e_scheduled_task"),
            None
        )

        # If no e2e_scheduled_task found, ensure at least one schedule exists
        if scheduled_workflow is None and len(schedules) > 0:
            scheduled_workflow = schedules[0]

        if scheduled_workflow:
            # Verify core workflow fields
            assert "id" in scheduled_workflow, "Should have id"
            assert "name" in scheduled_workflow, "Should have name"
            assert "schedule" in scheduled_workflow, "Should have schedule"

            # Verify enriched schedule fields (ScheduleMetadata)
            assert "validation_status" in scheduled_workflow, "Should have validation_status"
            assert scheduled_workflow["validation_status"] in ["valid", "warning", "error", None], \
                f"validation_status should be valid/warning/error/null, got {scheduled_workflow['validation_status']}"

            assert "human_readable" in scheduled_workflow, "Should have human_readable"
            assert "next_run_at" in scheduled_workflow, "Should have next_run_at"
            assert "is_overdue" in scheduled_workflow, "Should have is_overdue"
            assert isinstance(scheduled_workflow["is_overdue"], bool), "is_overdue should be bool"

            # Verify execution history fields
            assert "last_run_at" in scheduled_workflow, "Should have last_run_at"
            assert "last_execution_id" in scheduled_workflow, "Should have last_execution_id"
            assert "execution_count" in scheduled_workflow, "Should have execution_count"
            assert isinstance(scheduled_workflow["execution_count"], int), "execution_count should be int"

            # If this is a valid schedule, verify computed fields
            if scheduled_workflow["schedule"]:
                assert scheduled_workflow["validation_status"] == "valid", \
                    f"Valid cron should have valid status, got {scheduled_workflow['validation_status']}"
                assert scheduled_workflow["human_readable"] is not None, \
                    "Valid cron should have human_readable description"
                assert scheduled_workflow["next_run_at"] is not None, \
                    "Valid cron should have next_run_at"
